...
return [
     ...
     */
    // O default do pacote (id aleatório a cada request) vira o label "instance" no Grafana Cloud
    // e cria uma série de métrica nova por requisição — estoura o free tier em horas. gethostname()
    // dá um id estável por container (Sail e Laravel Cloud), da ordem de "1 por réplica".
    'service_instance_id' => env('OTEL_SERVICE_INSTANCE_ID', gethostname() ?: 'unknown'),
    ...
    'traces' => [
         ...
         */
        // Fixo em 'null', não por env: o escopo aqui é só métrica (requests + queries), sem
        // tracing distribuído. Precisa ser hardcoded porque OTEL_TRACES_EXPORTER=null no .env
        // vira `null` de PHP no env() do Laravel (não a string 'null'), e o default do pacote é
        // 'otlp' — sem isso, traces seriam empurrados junto sem ninguém ter pedido.
        'exporter' => 'null',
    ...
    'logs' => [
         ...
         * Supported drivers: "otlp", "console", "memory", "null"
         */
        // Fixo em 'null', mesmo motivo do traces.exporter acima — este projeto não tem pipeline
        // de logs (LOG_CHANNEL=stack/single) e o escopo pedido é só métrica.
        'exporter' => 'null',
    ...
    'instrumentation' => [
        ...
        // Desligado: app é event-sourced (spatie/laravel-event-sourcing), cada persist() dispara
        // eventos do framework — instrumentar isso é volume grande de spans/métricas que, com
        // traces.exporter='null', seriam só overhead sem nenhum consumidor.
        ...
        // Desligado: Laravel Scout nunca foi usado neste repo.
        Instrumentation\ScoutInstrumentation::class => false,
    ],
