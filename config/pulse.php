...
return [
    ...
    */
    ...
    // 'auth.basic' autentica contra a tabela users (e-mail + senha) e dá ao gate 'viewPulse' (ver
    // Modules/Identity/app/Providers/IdentityServiceProvider.php) um usuário pra checar. Não temos
    // tela de login de sessão neste app (é API-only pro front Angular), então é o jeito mais simples
    // de dar um usuário autenticado a esta tela Blade sem criar um fluxo de sessão à parte.
    'middleware' => [
        'web',
        'auth.basic',
        Authorize::class,
    ...
    /*
    |--------------------------------------------------------------------------
    | Pulse Allowed Emails
    |--------------------------------------------------------------------------
    |
    | E-mails autorizados a abrir o dashboard fora do ambiente local (ver o gate 'viewPulse' em
    | Modules/Identity/app/Providers/IdentityServiceProvider.php). Não existe conceito de papel/role
    | na v1 (decisão da F3), então a autorização em produção é por allowlist de e-mail via env.
    |
    */

    'allowed_emails' => array_filter(explode(',', (string) env('PULSE_ALLOWED_EMAILS'))),
    ...
    'recorders' => [
        ...
        Recorders\SlowRequests::class => [
            ...
            'ignore' => [
                ...
                '#^/livewire#', // Polling do próprio dashboard do Pulse (não é tráfego da aplicação)...
            ],
        ],
